...
def callback(msg):
    ...
    blob = cv2.dnn.blobFromImage(cvimage, 1 / 255.0, (416, 416),
    ...
    for i in range(len(boxes)):        
            ...
            if i in indexes:
                ...
                cv2.rectangle(cvimage, (x, y), (x + w, y + h), color, 2)

                cv2.putText(cvimage, label, (x, y -5),cv2.FONT_HERSHEY_SIMPLEX, 1/2, color, 2)

                print(posearray)
    
    
    cv2.imshow("Detections", cvimage)
...
if __name__=="__main__":
    ...
    detection_pub = rospy.Publisher('detection_topic2', PoseArray, queue_size=10)
